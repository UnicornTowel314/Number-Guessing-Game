...
CREATE_USER() {
CREATE_USER_RESULT=$($PSQL "INSERT INTO player_stats(username, games_played, best_game) VALUES('$USERNAME', 0, 0)")
  echo -e "\nWelcome, $USERNAME! It looks like this is your first time here."
}

FIND_USER() {
USER=$($PSQL "SELECT username FROM player_stats WHERE username='$USERNAME'")
...
if [[ $BEST_GAME -eq 0 ]] || [[ $BEST_GAME -gt $NUMBER_OF_GUESSES ]]
  then
  UPDATE_GAMES_PLAYED=$($PSQL "UPDATE player_stats SET best_game = $NUMBER_OF_GUESSES WHERE username='$USERNAME'")
fi
